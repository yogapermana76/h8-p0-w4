...
function countProfit(shoppers) {
  ...
  for(var i = 0; i < listBarang.length; i++) {
    ...
    for(var j = 0; j < shoppers.length; j++) {
        // cek jika pembeli membeli barang sesuai jenis barang
        if(info.product == shoppers[j].product && info.leftOver >= shoppers[j].amount) {
            info.shoppers.push(shoppers[j].name);
            info.leftOver -= shoppers[j].amount;
            info.totalProfit += (shoppers[j].amount * listBarang[i][1]);
        }
    }
    result.push(info);
  }
